...
class Queue:
    ...
    def enqueue(self, value: object) -> None:
        if self._sa.length() == self.size():
            temp_arr = StaticArray(self.size() * 2)
            pointer  = 0
            for index in range(self._front, self._sa.length()):
                temp_arr[pointer] = self._sa[index]
                pointer += 1
            for index in range(self._back + 1):
                temp_arr[pointer] = self._sa[index]
                pointer += 1
        
            self._sa = temp_arr
            self._front = 0
            self._back = pointer - 1
    ...
    def dequeue(self) -> object:
        ...
        dequeued_val = self._sa[self._front]
    ...
    def front(self) -> object:
        if self.is_empty():
            raise QueueException
        front_val = self._sa[self._front]
        return front_val 
